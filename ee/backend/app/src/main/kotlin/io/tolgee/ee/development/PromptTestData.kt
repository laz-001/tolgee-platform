...
class PromptTestData : BaseTestData() {
  ...
  init {
    organizationMember = root.addUserAccount {
      username = "member@organization.com"
    }
    organizationOwner = root.addUserAccount {
      username = "owner@organization.com"
    }

    unrelatedOrganization = root.addOrganization {
      name = "unrelated organization"
    }

    organization = root.addOrganization {
      ...
      llmProvider = addLLMProvider {
        name = "organization-provider"
      }
    }

    projectEditor = root.addUserAccount {
      username = "projectEditor@organization.com"
    }
    projectReviewer = root.addUserAccount {
      username = "projectReviewer@organization.com"
    }

    promptProject = root.addProject {
