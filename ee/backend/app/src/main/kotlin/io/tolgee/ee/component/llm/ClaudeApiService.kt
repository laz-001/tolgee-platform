...
class ClaudeApiService : AbstractLLMApiService(), Logging {
    override fun translate(
        ...
        if (params.shouldOutputJson) {
            inputMessages.add(
                LLMParams.Companion.LlmMessage(
                    LLMParams.Companion.LlmMessageType.TEXT, "Strictly return only valid json!"
                ),
            )
        }
        ...
        val requestBody = RequestBody(
            messages = messages,
            response_format = if (params.shouldOutputJson) {
                when (config.format) {
    ...
    companion object {
        ...
        class ResponseFormat(
            ...
            @JsonInclude(JsonInclude.Include.NON_NULL) val json_schema: Map<String, Any>? = mapOf(
